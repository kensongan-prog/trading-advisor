...
- `tests/` (286 pytest cases — the session-bootstrap test gate runs these)
- `rules/` (playbooks + risk doctrine)
- `journal/README.md` (the entry template + calibration cadence; the dir's real entries are git-ignored)
- `watchlist.example.md` (template — copy to `watchlist.md`, which is git-ignored; see Step 5)
- `portfolio.example.md` (template — copy to `portfolio.md` or just run `portfolio.py sync`; both git-ignored)
- `Trading Dashboard.command` (double-click launcher for `server.py`)

> [!warning] Personal trading data is **git-ignored, not in the repo.** `watchlist.md`, `portfolio.md`, and `journal/*.md` (except `README.md`) hold real positions/theses and never get committed — only the `*.example.md` templates are tracked. On a fresh clone you copy the templates to the real filenames (Step 5). See `.gitignore`.
...
- `watchlist.md` / `portfolio.md` / `journal/*.md` — these are your private records; start from the templates, don't copy someone else's
...
`watchlist.md` is git-ignored (your private list). Seed it from the tracked template, then add your own tickers across the three section bodies:

```bash
cp watchlist.example.md watchlist.md      # one-time, on a fresh clone
```
...
Edit via the dashboard's control-panel watchlist form (after the first build) or the CLI:

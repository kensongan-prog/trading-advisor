...
The 2026-07-07 launchd migration pointed the :8789 dashboard service at the GG-8-owned hermes-agent venv (`~/.hermes/hermes-agent/venv/bin/python`), which lacked yfinance+pandas; `server.py` spawns builds via `sys.executable` so every fetch hit `except ImportError → "yfinance not installed"` (returns BEFORE `cache_set`, so good caches survive — a refresh repopulates). Fix: `~/.hermes/hermes-agent/venv/bin/python -m pip install yfinance pandas`; live-verified :8789 then served real prices. **Recipe: `<the-server's-python> -c "import yfinance"` — if it fails, that's the whole bug.** Cross-agent watchpoint (GG-8 venv rebuild drops the deps) + full detail → vault Gotchas "Data builds need a yfinance+pandas-capable interpreter".
Enforced-by: **R4 closed 2026-07-06 (GG-8):** hermes-agent `pyproject.toml`/`uv.lock` pin `pandas==3.0.3`+`yfinance==1.5.1` (survives venv rebuild) + `gg8_dashboard_watchdog.py::_probe_build_deps` runs `<runtime_python> -c "import yfinance,pandas"` hourly per `dashboards.json` `build_deps` — trips the "HTTP 200 but every row errors" case before users see it. All 4 verified independently.
...
### 2026-07-04/05 — MooMoo `serve-dashboard` (:8788): launch detached via `-c`, not `-m` (else it dies silently)
tags: #tool:python #pattern:daemon-lifecycle #project:trading-advisor

Detached `python -m moomoo_adapter.cli serve-dashboard` dies instantly with an empty log; `-c` survives (bisected to the `-m` mode itself, not TCC). Working launch: `cd Projects/Codex/MooMoo && PYTHONPATH=src nohup .venv/bin/python -u -c "import sys; sys.argv=['moomoo-adapter','serve-dashboard']; from moomoo_adapter.cli import main; main()" </dev/null > /tmp/moomoo-dashboard.log 2>&1 & disown`. General rule (`-m`→`-c` before blaming TCC) promoted to the vault's `Lessons — Cross-Project.md`. Also: don't kill an operator-started server without saying so.
Enforced-by: — none yet
